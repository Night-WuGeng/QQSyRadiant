...
更新版本8.9.55.10980，发布全量、精简、净三个版本

全量和精简内置了模块，净采用精简底包进行了纯净打包，无内置，但净版本不可以再内置模块，否则出现的任何BUG不要找我反馈。

模块列表：QAuxiliary增强模块、QQcleaner清理模块、Qstory增强模块（仅内置作为QA补充，不打开个别功能对启动无影响）

A、全量：采用官方8.9.55.10980底包直接打包模块，如精简、净产生使用BUG建议安装全量
  
B、净：采用精简底包制作，具备精简性质但没有模块附加功能和其带来的影响，最强的冷启动速度和省电效率
  
C、精简：精简UE4引擎、精简无用库和监控库日志库、精简无用资源、精简无用组件、精简应用权限、精简服务及活动、减少广告、减少后台、减少游戏推广、加入防更新
